...
export const SITE_URL = "https://smartbill.dev";
...
export function localizedMetadata(language: SiteLanguage, path: string): Metadata {
  const copy = pageMeta[path as keyof typeof pageMeta]?.[language] ?? pageMeta["/"][language];
  ...
  return {
    ...
    openGraph: {
      ...
      images: [{ url: `${SITE_URL}${image}`, width: 1200, height: 630, type: "image/png", alt: copy.title }],
    },
    twitter: {
